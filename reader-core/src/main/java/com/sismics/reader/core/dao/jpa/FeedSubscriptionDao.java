...
public class FeedSubscriptionDao extends BaseDao<FeedSubscriptionDto, FeedSubscriptionCriteria> {
    ...
    protected QueryParam getQueryParam(FeedSubscriptionCriteria criteria, FilterCriteria filterCriteria) {
        ...
        if (criteria.getCategoryId() != null) {
            ...
            parameterMap.put(CATEGORY_ID, criteria.getCategoryId());
        }
    ...
    public FeedSubscription update(FeedSubscription feedSubscription) {
        EntityManager em = ThreadLocalContext.get().getEntityManager();
        String updateQuery="select fs from FeedSubscription fs where fs.id = :id and fs.deleteDate is null";
    ...
    public void updateUnreadCount(String id, Integer unreadCount) {
        EntityManager em = ThreadLocalContext.get().getEntityManager();
        String updateUnreadCount="update T_FEED_SUBSCRIPTION set FES_UNREADCOUNT_N = :unreadCount where FES_ID_C = :id";
        em.createNativeQuery(updateUnreadCount)
                .setParameter("id", id)
    ...
    public void reorder(FeedSubscription feedSubscription, int order) {
        EntityManager em = ThreadLocalContext.get().getEntityManager();
        String reorderQuery="select fs from FeedSubscription fs where fs.categoryId = :categoryId and fs.userId = :userId and fs.deleteDate is null order by fs.order";
        // Find categories with the same parent
        Query q = em.createQuery(reorderQuery)
                .setParameter(CATEGORY_ID, feedSubscription.getCategoryId())
                .setParameter(USER_ID, feedSubscription.getUserId());
    ...
    public void delete(String id) {
        EntityManager em = ThreadLocalContext.get().getEntityManager();
        String deleteQuery="select fs from FeedSubscription fs where fs.id = :id and fs.deleteDate is null";
        // Get the subscription
    ...
    public FeedSubscription getFeedSubscription(String id, String userId) {
        EntityManager em = ThreadLocalContext.get().getEntityManager();
        String getFeedSubscriptionQuery="select fs from FeedSubscription fs where fs.id = :id and fs.userId = :userId and fs.deleteDate is null";
        Query q = em.createQuery(getFeedSubscriptionQuery)
                .setParameter("id", id)
    ...
    public List<FeedSubscription> findByCategory(String categoryId) {
        EntityManager em = ThreadLocalContext.get().getEntityManager();
        String findByCategoryQuery="select fs from FeedSubscription fs where fs.categoryId = :categoryId and fs.deleteDate is null";
        Query q = em.createQuery(findByCategoryQuery)
    ...
    public int getCategoryCount(String categoryId, String userId) {
        EntityManager em = ThreadLocalContext.get().getEntityManager();
        String getCategoryCountQuery="select count(fs.id) from FeedSubscription fs where fs.categoryId = :categoryId and fs.userId = :userId and fs.deleteDate is null";
        Query q = em.createQuery(getCategoryCountQuery)
                .setParameter(CATEGORY_ID, categoryId)
                .setParameter(USER_ID, userId);
