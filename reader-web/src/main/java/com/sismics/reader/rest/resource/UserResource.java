...
public class UserResource extends BaseResource {
    ...
    private void validateInputs(String password, String email, String themeId, String localeId) throws JSONException {
        ...
        themeId = com.sismics.reader.rest.util.ValidationUtil.validateTheme(
                EnvironmentUtil.isUnitTest() ? null : request.getServletContext(), themeId, "theme", true);
    }

    private void updateUserFields(User user, String email, String themeId, String localeId,
    ...
    private void updatePasswordIfNeeded(UserDao userDao, User user, String password) {
        if (StringUtils.isNotBlank(password)) {
            user.setPassword(password);
            user = userDao.updatePassword(user);

            // Raise Password Change Event
    ...
    public Response update(
        ...
        // Validate Inputs
        validateInputs(password, email, themeId, localeId);

        // Fetch and Update User
        UserDao userDao = new UserDao();
        User user = userDao.getActiveByUsername(principal.getName());
        updateUserFields(user, email, themeId, localeId, displayTitleWeb, displayTitleMobile,
                displayUnreadWeb, displayUnreadMobile, narrowArticle, firstConnection);

        user = userDao.update(user);

        // Update Password if provided
        updatePasswordIfNeeded(userDao, user, password);
    ...
    public Response update(
        ...
        updateUserFields(user, email, themeId, localeId, displayTitleWeb, displayTitleMobile,
                displayUnreadWeb, displayUnreadMobile, narrowArticle, null);
    ...
    public Response info() throws JSONException {
        ...
        if (!authenticate()) {
            getAnonymousUserInfo(response);
        } else {
            getAuthenticatedUserInfo(response);
        }

        return Response.ok().entity(response).build();
    }

    private void getAnonymousUserInfo(JSONObject response) throws JSONException {
        response.put("anonymous", true);
        response.put("locale", LocaleUtil.getLocaleIdFromAcceptLanguage(request.getHeader("Accept-Language")));
    ...
    private void getAuthenticatedUserInfo(JSONObject response) throws JSONException {
        ...
        fetchUserJobs(response, user);
    }

    private void populateUserInfo(JSONObject response, User user) throws JSONException {
        ...
        response.put("base_functions", new JSONArray(((UserPrincipal) principal).getBaseFunctionSet()));
    ...
    private void fetchUserJobs(JSONObject response, User user) throws JSONException {
        JobDao jobDao = new JobDao();
        JobEventDao jobEventDao = new JobEventDao();

        List<JobDto> jobList = jobDao.findByCriteria(new JobCriteria().setUserId(user.getId()));
        JSONArray jobs = new JSONArray();

        for (JobDto job : jobList) {
            JSONObject jobJson = createJobJson(job, jobEventDao);
            jobs.put(jobJson);
        }
    ...
    private JSONObject createJobJson(JobDto job, JobEventDao jobEventDao) throws JSONException {
        ...
        List<JobEventDto> jobEventList = jobEventDao.findByCriteria(new JobEventCriteria().setJobId(job.getId()));
        populateJobEventCounts(jobJson, jobEventList);

        return jobJson;
    }

    private void populateJobEventCounts(JSONObject jobJson, List<JobEventDto> jobEventList) throws JSONException {
        int feedSuccess = 0, feedFailure = 0, starredSuccess = 0, starredFailure = 0;

        for (JobEventDto jobEvent : jobEventList) {
            String name = jobEvent.getName();
            int value = Integer.parseInt(jobEvent.getValue());

            switch (name) {
                case Constants.JOB_EVENT_FEED_COUNT:
                    jobJson.put("feed_total", value);
                    break;
                case Constants.JOB_EVENT_STARRED_ARTICLED_COUNT:
                    jobJson.put("starred_total", value);
                    break;
        ...
        jobJson.put("feed_success", feedSuccess);
        jobJson.put("feed_failure", feedFailure);
        jobJson.put("starred_success", starredSuccess);
        jobJson.put("starred_failure", starredFailure);
    }
